            ...
            {list.map((item, i) => (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1 mb-2">
                  <h3 className="text-[1.05rem] font-semibold text-white">{item.role}</h3>
                  <span className="text-[0.75rem] text-[#444] font-mono shrink-0">
                    {item.period}
                  </span>
                </div>
                <p className="text-[0.85rem] text-accent/60 mb-3 font-medium">{item.company}</p>
                <p className="text-[0.9rem] text-[#666] leading-relaxed mb-4">{item.summary}</p>
                <ul className="space-y-2">
                  {item.bullets.map((bullet, j) => (
                    <li key={j} className="flex items-start gap-3 text-[0.85rem] text-[#555]">

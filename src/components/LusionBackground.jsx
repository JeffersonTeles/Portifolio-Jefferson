  ...
  useFrame((state) => {
    ...
    if (gridRef.current) {
      ...
      gridRef.current.position.z =
        -4 + Math.sin(time + mouse.x) * config.physics;
    }
...
const LusionBackground = ({ isDark, isStealth }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[-1] pointer-events-none transition-opacity duration-1000 ${isStealth ? "opacity-10 grayscale" : "opacity-100"}`}
    >
      <Canvas
